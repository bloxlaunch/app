	...
	useEffect(() => {
		async function fetchGameData() {
			try {
				...
				const universeResponse = await fetch(`https://apis.roproxy.com/universes/v1/places/${id}/universe`);
				...
				const [iconResponse, bannerResponse] = await Promise.all([
					fetch(`https://thumbnails.roproxy.com/v1/places/gameicons?placeIds=${id}&returnPolicy=PlaceHolder&size=512x512&format=Webp&isCircular=false`),
					fetch(`https://thumbnails.roproxy.com/v1/assets?assetIds=${id}&returnPolicy=PlaceHolder&size=1320x440&format=Png&isCircular=false`),
				]);
